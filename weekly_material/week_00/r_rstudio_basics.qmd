...
1.  To access the Public Health Datahub, please use the link on the Datahub tab within the course bCourses site. This link will automatically sync to provide the most updated course material. *Note: there are other Datahubs at UC Berkeley, however the setup is slightly different. Please use the one linked on bCourses.*
2.  Sign in with bCourses, if prompted (you may not be prompted to since you are already within bCourses). You will need to click "Authorize" to allow Datahub to connect to your bCourses account.
