---
title: "PHW251 Live Session - October 7, 2025"
format: html
editor: visual
---
...
We are going to do just a bit of review on creating new objects and new variables within data frames and discuss the syntax of this code.
...
2.  Is there an issue with the code below? If so, what is it?
...
3.  What if both patients who got the cider treatment and patients who got the purgative mixture as treatment died? Try creating this new variable two ways:

    a\. Using dplyr methods `if_else` and `mutate`

    b\. Using dplyr methods `case_when` and `mutate`

```{r}
#if_else
scurvy1 <- scurvy %>%
  mutate(died = if_else(treatment %in% c("purgative_mixture","cider"),1,0))
...
ANSWER: It can be harder to keep track of what you are doing, or to find errors when your code isn't working. These concerns can be avoided by frequently running sections of the code as you write them, for example, highlight and run the first line and the rename_with line to ensure it works correctly. Then try adding in the rename line, etc.
